...
(function() {
  var KEY_CODE, SELECTION, deriv, dx, fixed_point, handle, newton_transform, newtons_method, tolerance, workspace;
  ...
  fixed_point = function(f, first_guess) {
    var close_enough, try_it;
    close_enough = function(v1, v2) {
      return Math.abs(v1 - v2) < tolerance;
    };
    try_it = function(guess) {
      var next;
      next = f(guess);
  ...
  handle = function(addedLetter) {
    var _ref;
    if ((KEY_CODE['min_num'] <= (_ref = addedLetter.charCodeAt(0)) && _ref <= KEY_CODE['max_num'])) {
      return console.log('A Number');
    }
  };

  $(workspace).on('focus', function() {
    var $this;
    $this = $(this);
    ...
    var addedText, e, location, node, raw, _ref;
    raw = $(workspace).text();
    if (!$(workspace).children('.computation')) {
      e = document.createElement('span');
    }
    _ref = SELECTION.getStart(), node = _ref[0], location = _ref[1];
    addedText = $(node).text()[location - 1];
    return handle(addedText);
    /*
          $(workspace).html('')    
          out = ""
          index = 0
